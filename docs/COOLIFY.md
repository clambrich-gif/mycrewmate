...
| `ER_CANT_AGGREGATE_2COLLATIONS` bei `0014_fat_dracula`  | Log zeigt den Vergleich `events` ↔ `app_settings`                | Mit diesem Stand behoben: Die historische Logo-Übernahme gleicht Name und Jahr kollationsunabhängig binär ab; keine globale DB-Kollation ändern.    |
| `ER_PARSE_ERROR` nahe `INSERT INTO prep_tasks` bei `0053` | Log zeigt eine Transaktion mit mehreren Anweisungen              | Mit diesem Stand behoben: Jede Anweisung besitzt nun einen Drizzle-Statement-Breakpoint; die Transaktion bleibt vollständig erhalten.               |
| `ER_UNSUPPORTED_PS` bei `START TRANSACTION` in `0053`   | Log nennt das MySQL Prepared-Statement-Protokoll                 | Mit diesem Stand behoben: Vertrauenswürdige, eingecheckte Migrations-SQL läuft über das MySQL-Textprotokoll; Parameterabfragen bleiben vorbereitet. |
