...
interface Props {
  kind: string; title: string; addLabel: string; nameKey: string;
  columns: Col[];
  statusField?: boolean;
  statusOptions?: { v: string; l: string }[];
  extraField?: { key: string; label: string; options: { v: string; l: string }[] };
  noContact?: boolean; noStatus?: boolean;
}

export default function TaskGeneric({ kind, title, addLabel, nameKey, columns, statusField, statusOptions, extraField, noContact, noStatus }: Props) {
  ...
  const create = api.create.useMutation({ onSuccess: () => { invalidate(); setName(""); setExtras({}); toast.success("Hinzugefügt"); } });
  ...
  const defaultStatus = statusOptions ?? [{ v: "offen", l: "offen" }, { v: "inArbeit", l: "in Arbeit" }, { v: "erledigt", l: "erledigt" }];
    ...
    <div className="space-y-5">
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Input placeholder={`Neu: ${addLabel}`} value={name} onChange={e => setName(e.target.value)} className="w-64" onKeyDown={e => e.key === "Enter" && name.trim() && create.mutate({ [nameKey]: name.trim() })} />
          {columns.map(c => (
            <Input key={c.key} placeholder={c.label} value={extras[c.key] ?? ""} onChange={e => setExtras({ ...extras, [c.key]: e.target.value })} className="w-36" />
          ))}
          <Button onClick={() => name.trim() && create.mutate({ [nameKey]: name.trim(), ...extras })}>Hinzufügen</Button>
        </div>
      </div>
      <Card className="shadow-sm"><CardContent className="p-0 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
                ...
                {columns.map(c => (
                  <td key={c.key} className="p-2">
                    <Input className="h-8 w-full min-w-[90px]" defaultValue={r[c.key] ?? ""}
                      onBlur={e => { if (e.target.value !== (r[c.key] ?? "")) update.mutate({ id: r.id, [c.key]: e.target.value }); }} />
                  </td>
                ))}
                {!noContact && (
                  <td className="p-2">
                ...
                {!noStatus && (
                  <td className="p-2">
                    <Select value={r.status} onValueChange={v => update.mutate({ id: r.id, status: v })}>
                      ...
                      <SelectContent>{defaultStatus.map(o => <SelectItem key={o.v} value={o.v}>{o.l}</SelectItem>)}</SelectContent>
                    </Select>
                  </td>
                )}
                {extraField && (
                  <td className="p-2">
                    <Select value={r[extraField.key]} onValueChange={v => update.mutate({ id: r.id, [extraField.key]: v })}>
                      <SelectTrigger className="h-8 w-[110px]"><SelectValue /></SelectTrigger>
                      <SelectContent>{extraField.options.map(o => <SelectItem key={o.v} value={o.v}>{o.l}</SelectItem>)}</SelectContent>
                    </Select>
                  </td>
                )}
            ...
            {!isLoading && rows.length === 0 && <tr><td className="p-4 text-muted-foreground" colSpan={8}>Noch keine Einträge.</td></tr>}
          </tbody>
        </table>
      </CardContent></Card>
    </div>
