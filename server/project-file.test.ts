...
describe("Projektdatei und modularer Excel-Import", () => {
  ...
  it("migriert v15-Dateien mit Kuchenspenden auf Version 16 mit neutraler Kategorie", async () => {
    const exported = await exportProjectFile();
    const legacyV15 = parseProjectFile(exported.buffer.toString("base64")).document;
    legacyV15.metadata.version = 15;
    (legacyV15 as any).cakes = [
      {
        sourceId: 993,
