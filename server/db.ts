...
export async function listContacts() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contacts).orderBy(contacts.sortOrder, contacts.name);
}
export async function listHelpers() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(helpers).orderBy(helpers.name);
}
export async function listShifts() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(shifts).orderBy(shifts.sortOrder, shifts.id);
}
...
export async function listPrep() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(prepTasks).orderBy(prepTasks.sortOrder, prepTasks.id);
}
export async function listPost() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(postTasks).orderBy(postTasks.sortOrder, postTasks.id);
}
export async function listMaterials() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(materials).orderBy(materials.sortOrder, materials.id);
}
export async function listMarketing() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(marketing).orderBy(marketing.sortOrder, marketing.id);
}
export async function listApprovals() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(approvals).orderBy(approvals.sortOrder, approvals.id);
}
export async function listCakes() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(cakes).orderBy(cakes.sortOrder, cakes.id);
}
...
export async function createHelper(
  ...
  const db = (await getDb()) as DB;
  return db.insert(helpers).values(v as typeof helpers.$inferInsert);
}
export async function updateHelper(
  ...
  const db = (await getDb()) as DB;
  await db.update(helpers).set(v).where(eq(helpers.id, id));
}
export async function deleteHelper(id: number) {
  const db = (await getDb()) as DB;
  await db.delete(helpers).where(eq(helpers.id, id));
}
...
export async function createShift(
  ...
  const db = (await getDb()) as DB;
  return db.insert(shifts).values(v as typeof shifts.$inferInsert);
}
export async function updateShift(
  ...
  const db = (await getDb()) as DB;
  await db.update(shifts).set(v).where(eq(shifts.id, id));
}
...
export const createPrep = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(prepTasks).values(v);
};
export const updatePrep = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(prepTasks).set(v).where(eq(prepTasks.id, id));
};
export const deletePrep = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(prepTasks).where(eq(prepTasks.id, id));
};
export const createPost = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(postTasks).values(v);
};
export const updatePost = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(postTasks).set(v).where(eq(postTasks.id, id));
};
export const deletePost = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(postTasks).where(eq(postTasks.id, id));
};
export const createMaterial = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(materials).values(v);
};
export const updateMaterial = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(materials).set(v).where(eq(materials.id, id));
};
...
export const createMarketing = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(marketing).values(v);
};
export const updateMarketing = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(marketing).set(v).where(eq(marketing.id, id));
};
export const deleteMarketing = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(marketing).where(eq(marketing.id, id));
};
export const createApproval = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(approvals).values(v);
};
export const updateApproval = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(approvals).set(v).where(eq(approvals.id, id));
};
export const deleteApproval = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(approvals).where(eq(approvals.id, id));
};
export const createCake = async (v: any) => {
  const db = (await getDb()) as DB;
  return db.insert(cakes).values(v);
};
export const updateCake = async (id: number, v: any) => {
  const db = (await getDb()) as DB;
  return db.update(cakes).set(v).where(eq(cakes.id, id));
};
export const deleteCake = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(cakes).where(eq(cakes.id, id));
};
type FinanceWrite = {
  category?: string;
  incomeCents?: number;
...
export const createFinance = async (v: FinanceWrite & { category: string }) => {
  ...
  return db
    .insert(finances)
    .values(mapFinanceWrite(v) as typeof finances.$inferInsert);
};
export const updateFinance = async (id: number, v: FinanceWrite) => {
  const db = (await getDb()) as DB;
  return db.update(finances).set(mapFinanceWrite(v)).where(eq(finances.id, id));
};
export const deleteFinance = async (id: number) => {
  const db = (await getDb()) as DB;
  return db.delete(finances).where(eq(finances.id, id));
};
