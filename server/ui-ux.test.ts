...
describe("UI- und Mobile-UX-Regeln", () => {
  ...
  it("richtet die Helferplanung als installierbare PWA mit mobilem Installationshinweis ein", () => {
    ...
    expect(layout).toContain("Als App speichern");
    expect(layout).not.toContain("Als App auf Handy speichern");
    expect(layout).toContain('min-h-11 w-full min-w-0 items-center justify-start overflow-hidden');
    expect(layout).toContain('min-w-0 flex-1 whitespace-normal text-left text-sm leading-tight');
    expect(layout).toContain('mr-2 h-10 w-10 shrink-0 rounded-xl bg-white p-1.5 object-contain shadow-md');
  ...
  it("bietet zentrale Orte, Ortsauswahl und Kartenlinks in Schichten und Vorbereitungen", () => {
    ...
    expect(locationDetails).toContain("activeTab: LocationDetailTab");
    expect(locationDetails).toContain("onActiveTabChange");
    expect(locationDetails).toContain('value={activeTab}');
    expect(locationDetails).toContain("onActiveTabChange(value as LocationDetailTab)");
    ...
    expect(mapClient).toContain("LOCATION_DETAIL_TAB_STORAGE_PREFIX");
    expect(mapClient).toContain("storedLocationDetailTab");
    expect(mapClient).toContain("detailTabsByLocationId");
    expect(mapClient).toContain("activeTabForLocation");
    expect(mapClient).toContain("rememberLocationDetailTab");
    expect(mapClient).toContain("window.sessionStorage.setItem");
  ...
  it("bindet im Zugangsschutz das Verwaltungsmodul für Planungsteam-Zugänge ein", () => {
    ...
    expect(manager).toContain("<form className=\"min-w-0 space-y-4\" onSubmit={submitDelete}>");
    expect(manager).toContain("!flex !flex-col-reverse !gap-3 sm:!flex-row sm:!items-center sm:!justify-end");
    expect(manager).toContain('type="submit"');
    expect(manager).toContain("w-full shrink-0 whitespace-nowrap");
    expect(manager).toContain("bg-red-600");
    expect(manager).toContain("hover:bg-red-700");
    expect(manager).toContain("text-white");
    expect(manager).toContain("opacity-100 transition-colors");
    expect(manager).toContain("Zugangsdaten löschen");
    expect(manager).not.toContain("Zugangsdaten dauerhaft löschen");
    expect(manager).toContain("!overflow-x-hidden overflow-y-auto");
    expect(manager).toContain('className="w-full max-w-full"');
