...
async function buildModuleTarget(
  base64: string,
  area: ModuleImportArea,
  baseDocument?: BackupDocument
) {
  const current = baseDocument ?? (await createCurrentProjectDocument());
...
/** Alle bisher möglichen Modulimporte stehen zentral in der Seitenleiste bereit. */
export const CENTRAL_MODULE_IMPORT_AREAS = [
  "ANSPRECHPARTNER",
  "HELFER",
  "EINSATZPLAN",
  "VORBEREITUNG",
...
export type CentralModuleImportArea =
  (typeof CENTRAL_MODULE_IMPORT_AREAS)[number];

const centralAreaName: Record<CentralModuleImportArea, string> = {
  ANSPRECHPARTNER: "Ansprechpartner",
  HELFER: "Helfer",
  EINSATZPLAN: "Einsatzplan",
  VORBEREITUNG: "Vorbereitung",
...
function normalizeSelectedAreas(areas: readonly CentralModuleImportArea[]) {
  const selected = Array.from(new Set(areas));
  if (!selected.length) {
    throw new Error("Bitte mindestens einen Excel-Bereich auswählen");
  }
  return CENTRAL_MODULE_IMPORT_AREAS.filter(area => selected.includes(area));
}

async function buildSelectedModuleTarget(
  base64: string,
  requestedAreas: readonly CentralModuleImportArea[]
) {
  const areas = normalizeSelectedAreas(requestedAreas);
  let target = await createCurrentProjectDocument();
  let rowsChecked = 0;
  const warnings: string[] = [];

  // Jede Folgeprüfung verwendet bewusst das Ergebnis der vorigen Auswahl.
  // Dadurch bleiben zwingende Verknüpfungen (z. B. Ansprechpartner → eigene
  // Helferzeile → Schichtzuweisung) auch bei einer Mehrfachauswahl konsistent.
  for (const area of areas) {
    const result = await buildModuleTarget(base64, area, target);
    target = result.target;
    rowsChecked += result.rowsChecked;
    warnings.push(...result.target.warnings);
  }

  target.warnings = Array.from(new Set(warnings));
  return { areas, target, rowsChecked };
}

export async function previewSelectedModuleExcelImport(
  base64: string,
  requestedAreas: readonly CentralModuleImportArea[]
) {
  const { areas, target, rowsChecked } = await buildSelectedModuleTarget(
    base64,
    requestedAreas
  );
  const preview = await previewProjectDocument(target, digest(base64));
  return {
    areas,
    areaNames: areas.map(area => centralAreaName[area]),
    currentDigest: preview.currentDigest,
    sourceDigest: preview.workbookDigest,
    warnings: preview.warnings,
    changes: preview.changes,
    totals: summarize(preview.changes),
    rowsChecked,
  };
}

export async function applySelectedModuleExcelImport(
  base64: string,
  requestedAreas: readonly CentralModuleImportArea[],
  filename: string,
  ...
  const { areas, target } = await buildSelectedModuleTarget(base64, requestedAreas);
  const changes = await previewProjectDocument(target, digest(base64));
  if (!changes.changes.length) {
    throw new Error("Die Excel-Datei enthält für die Auswahl keine Änderungen");
  }
  return restoreProjectDocument(
    target,
    digest(base64),
    `${areas.map(area => centralAreaName[area]).join(", ")} · ${filename}`,
